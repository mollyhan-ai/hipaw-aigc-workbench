...
export function CanvasTopBar({
    ...
    if (embedded) return <div className="absolute left-0 right-0 top-0 z-50 flex h-12 items-center justify-between gap-2 px-3" style={{background:theme.canvas.background,color:theme.toolbar.item}}>
        <div className="flex min-w-0 items-center gap-1 text-sm">
            <button className="grid size-8 shrink-0 place-items-center rounded-lg hover:bg-black/5 dark:hover:bg-white/10" onClick={toggleSidePanel} aria-label="切换画布图层" title="图层"><PanelLeftOpen size={17}/></button>
            <button className="grid size-8 shrink-0 place-items-center rounded-lg hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-30" onClick={onUndo} disabled={!canUndo} aria-label="撤销" title="撤销"><Undo2 size={17}/></button>
            <button className="grid size-8 shrink-0 place-items-center rounded-lg hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-30" onClick={onRedo} disabled={!canRedo} aria-label="重做" title="重做"><Redo2 size={17}/></button>
            <button className="flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-2 hover:bg-black/5 dark:hover:bg-white/10" onClick={onImportImage} aria-label="上传图片" title="上传图片"><Upload size={16}/><span className="hidden sm:inline">上传</span></button>
            <button className="flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-2 hover:bg-black/5 dark:hover:bg-white/10" onClick={onExportProject} aria-label="导出画布" title="导出画布"><Download size={16}/><span className="hidden sm:inline">导出</span></button>
        </div>
        <a href="https://github.com/basketikun/infinite-canvas" target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs opacity-60 hover:opacity-100" title="画布基于 basketikun / infinite-canvas">infinite-canvas</a>
    </div>;
    return (
        <>
