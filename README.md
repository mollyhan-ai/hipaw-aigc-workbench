...
页面采用**深绿色导航、米白背景、白色面板和黄色强调**，恢复 HiPaw 原版视觉。桌面端将生图设置放在画布左侧，支持专注模式；资产详情保持点击打开弹窗。
...
[打开 HiPaw 工作台](https://mollyhan-ai.github.io/hipaw-aigc-workbench/)

GitHub Pages 展示当前原型界面，支持浏览流程与编辑、导出画布；病例及内容为示例。品牌资料展示默认配置，真实生图、品牌保存、设计素材上传与作品入库需要本机后端。网页不接收 API 密钥，也不包含本机数据；画布编辑保存在当前浏览器。

推送到 main 后，GitHub Actions 自动构建并发布。手动构建使用 `npm run build:pages`，输出为 `_site/`；不会覆盖本机画布构建。仅打包前端文件、默认品牌配置与画布，不包含 `data/`、密钥或私人品牌资料。
